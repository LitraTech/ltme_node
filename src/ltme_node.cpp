...
void LidarDriver::run()
{
  ...
  ros::ServiceServer set_scan_frequency_service = nh_private_.advertiseService<ltme_node::SetScanFrequencyRequest, ltme_node::SetScanFrequencyResponse>
    ("set_scan_frequency", std::bind(&LidarDriver::setScanFrequency, this, std::placeholders::_1, std::placeholders::_2));
  ...
  while (nh_.ok() && !quit_driver_.load()) {
    if (device_->open() == ldcp_sdk::no_error) {
      ...
      if (!reboot_required) {
        if (scan_frequency_override_ != 0)
          active_scan_frequency_ = scan_frequency_override_;
        else {
          int scan_frequency = 0;
          if (device_->getScanFrequency(scan_frequency) == ldcp_sdk::no_error)
            active_scan_frequency_ = scan_frequency;
          else {
            active_scan_frequency_ = DEFAULT_SCAN_FREQUENCY;
            ROS_WARN("Unable to query device for scan frequency and will use %d as the frequency value", active_scan_frequency_.load());
          }
        }

        int target_scan_frequency = -1;
        if (nh_private_.getParam("scan_frequency", target_scan_frequency)) {
          if (device_model_ == "LTME-02A")
            ROS_WARN("LTME-02A doesn't support dynamic scan frequency.");
          else if (active_scan_frequency_.load() == target_scan_frequency)
            ROS_INFO("Device already scanning at %d Hz", target_scan_frequency);
          else {
            switch (target_scan_frequency) {
              case 10:
              case 15:
              case 20:
              case 25:
              case 30:
                if (device_->setScanFrequency(target_scan_frequency) == ldcp_sdk::no_error) {
                  active_scan_frequency_ = target_scan_frequency;
                  ROS_INFO("Scan frequency changed to %d Hz", target_scan_frequency);
                }
                else
                  ROS_WARN("Failed to change scan frequency");
                break;
              default:
                ROS_WARN("Unsupported scan frequency (%d Hz)", target_scan_frequency);
                break;
            }
          }
        }
        ...
        while (nh_.ok() && !quit_driver_.load()) {
          ...
          try {
            ...
            laser_scan.time_increment = 1.0 / active_scan_frequency_.load() / beam_count * average_factor_;
            laser_scan.scan_time = 1.0 / active_scan_frequency_.load();
...
bool LidarDriver::setScanFrequency(ltme_node::SetScanFrequencyRequest& request, ltme_node::SetScanFrequencyResponse& response)
{
  bool result = false;
  std::unique_lock<std::mutex> lock(mutex_, std::try_to_lock);
  if (lock.owns_lock()) {
    if (active_scan_frequency_.load() == request.target_scan_frequency) {
      ROS_INFO("Device already scanning at %d Hz", request.target_scan_frequency);
      result = true;
    }
    else {
      switch (request.target_scan_frequency) {
        case 10:
        case 15:
        case 20:
        case 25:
        case 30:
          if (device_->setScanFrequency(request.target_scan_frequency) == ldcp_sdk::no_error) {
            active_scan_frequency_ = request.target_scan_frequency;
            ROS_INFO("Scan frequency changed to %d Hz", request.target_scan_frequency);
            result = true;
          }
          else
            ROS_WARN("Failed to change scan frequency");
          break;
        default:
          ROS_WARN("Unsupported scan frequency (%d Hz)", request.target_scan_frequency);
          break;
      }
    }
  }
